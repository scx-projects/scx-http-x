...
public final class HttpClientRequest extends AbstractHttpSender<ScxHttpClientResponse> implements Http1ClientRequest, Http2ClientRequest {
    ...
    protected ScxHttpClientResponse send0(BodyWriter bodyWriter) throws ScxHttpSendException, ScxWrappedException, ScxHttpReceiveException {
        ...
        if (useHttp2) {
            var http2ClientConnection = new Http2ClientConnection(endpoint, options.http2ClientConnectionOptions());
            http2ClientConnection.sendRequest(this, bodyWriter);
            return http2ClientConnection.readResponse();
        } else {
            // 仅当 http 协议 (不是 SSL) 并且开启代理的时候才使用 绝对路径
            if (!(endpoint.socket instanceof SSLSocket) && options.proxy() != null) {
                this.useProxy = true;
            }
            var http1ClientConnection = new Http1ClientConnection(endpoint, options.http1ClientConnectionOptions());
            var requestLine = http1ClientConnection.sendRequest(this, bodyWriter);
            return http1ClientConnection.readResponse(requestLine);
        }
