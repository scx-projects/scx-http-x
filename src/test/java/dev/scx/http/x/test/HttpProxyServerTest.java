    ...
    public static void test1() throws IOException {
        ...
        httpServer.onRequest(c -> {
            ...
            if (c.method() == HttpMethod.CONNECT) {
                ...
                try {
                    // 使用较短的 超时
                    clientTCPSocket = tcpClient.connect(new InetSocketAddress(c.uri().host(), c.uri().port()), 2000);
                } catch (IOException e) {
                ...
                // 3, 获取 当前连接的 底层 tcpSocket
                var serverTCPSocket = request.connection.endpoint.socket;
                ...
                // 使用较短的 超时
                var httpClient = new HttpClient(new HttpClientOptions().timeout(2000));
